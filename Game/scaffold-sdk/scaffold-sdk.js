;(function (root) {
  ...
  var api = {
    ...
    mail: {
      ...
      claimAll: function () {
        return authedRequest('POST', '/mail/claim-all');
      },
    },
    rank: {
      ...
      getDimensions: function (rankType, options) {
        options = options || {};
        return authedRequest('GET', '/rank/' + segment(rankType) + '/dimensions', undefined, {
          period: options.period,
        });
      },
    },
    redemption: {
      redeem: function (code) {
        var text = typeof code === 'string' ? code.trim() : '';
        if (!text) {
          return Promise.reject(createError(400, 'code is required'));
        }
        return authedRequest('POST', '/redemption/redeem', { code: text });
      },
    },
    realtime: {
